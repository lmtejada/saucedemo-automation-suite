...
| **CI/CD**         | Push: lint + typecheck + smoke.<br>Push to `main`: full regression + e2e + a11y gate in Chromium.<br>PR to `main`: full suite across all 3 browsers.                                                    |
...
| **Git workflow**  | Conventional Commits (`feat:`, `fix:`, `chore:`, ...) enforced locally via Husky (`commit-msg`); `feat/`, `fix/`, `release/`, `epic/` branch prefixes enforced via a `pre-push` hook + a GitHub ruleset |
